    ...
    entries.forEach(entry => {
      if(entry.isIntersecting){
        if(entry.target.getAttribute('lazy-background')){
          entry.target.style.backgroundImage = `url(${entry.target.getAttribute('lazy-background')})`
          entry.target.removeAttribute('lazy-background');
          if(options.loadedClass !== false) entry.target.classList.add(options.loadedClass)
        } else {
...
const vLazyBackground = Vue.directive('lazy-background', {
  ...
  set(el) {
    ...
    if(process.browser) observer.observe(el);
  }
});
