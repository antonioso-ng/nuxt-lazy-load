...
const vLazyLoad = Vue.directive('lazy-load', {
  ...
  set(el, value) {
    ...
    if(process.browser){
      ...
      if(['notLazy', 'noLazy'].some(d => element.dataset[d] !== undefined)){
        element.removeAttribute('data-src');
        element.removeAttribute('data-no-lazy');
        element.removeAttribute('data-not-lazy');
      } else{
        if(value){
          let media = [];
          ...
          for(let m of media){
            if(options.appendClass !== false) m.classList.add(options.appendClass);
            observer.observe(m);
          }
        } else{
          if(options[elementsMap[element.tagName.toLowerCase()]] !== true){
